...
//// for unidirectional cases
point getListOfPotentialWireBlocks_uni(point, wireBlock**, int, vector<point>*, int iteration, point Target);

int doPropagate_uni(vector<point> list, point Target, int tracksPerChannel, wireBlock** wb, int iteration, int wireIndex) {
    ...
    for (int i = 0; i < list.size(); i++) {
        ...
        point retVal = getListOfPotentialWireBlocks_uni(list[i], wb1, wireIndex, &potentialList, iteration, Target);
        cout << "propagation #" << i << " done" << endl;
        if (retVal.i == -1 && retVal.j == -1) {
            //we have no match for target, lets move ahead with the new list
            // this is a NO-OP
    ...
    //if no match was found, and its not a dead end, we shall now move ahead
    returnStatus = doPropagate_uni(potentialList, Target, tracksPerChannel, wb1, iteration + 1, wireIndex);
