...
vector<int> pin;
vector<vector<wbpoint>> route_uni;
...
int DrawNow_uni(int _gridSize, int _tracksPerChannel, vector<vector<wbpoint>> _route, vector<int> _sourcePin, vector<int> _targetPin, int _track,  vector<point> _sourceWB, vector<point> _targetWB){
    cout<<"Hello Graphics\n";
...
//    }
    ...
    event_loop(NULL, NULL, NULL, drawscreen_uni);  
//    drawscreen();
//    flushinput();
//    cin.get();
    close_graphics();
	...
	//setcolor (rand()%255, rand()%255, rand()%255);
        ...
        for (int s = 0; s < route_uni.size(); s++){
            ...
            for (int r = 0; r < route_uni[s].size(); r++){
                    if (route_uni[s][r].i % 2 == 1 && route_uni[s][r].j % 2 == 0){
                            //horizontal lines
                            drawline(rectOffset+BlockDim*route_uni[s][r].i, rectOffset+BlockDim*route_uni[s][r].j + wireOffset*(route_uni[s][r].wirenumber +1), rectOffset+BlockDim*(route_uni[s][r].i+1), rectOffset + BlockDim*route_uni[s][r].j + wireOffset*(route_uni[s][r].wirenumber+1));
                    }
                    if (route_uni[s][r].i % 2 == 0 && route_uni[s][r].j % 2 == 1){
                            //vertical lines
                            drawline(rectOffset+BlockDim*route_uni[s][r].i + wireOffset*(route_uni[s][r].wirenumber+1), rectOffset+BlockDim*route_uni[s][r].j, rectOffset+BlockDim*route_uni[s][r].i + wireOffset*(route_uni[s][r].wirenumber+1), rectOffset+BlockDim*(route_uni[s][r].j+1));
                    }
            }
            drawX(sourcePin[s], route_uni[s][r].wirenumber, SourceWB[s], BlockDim, wireOffset, rectOffset);
            drawX(targetPin[s], route_uni[s][r].wirenumber, TargetWB[s], BlockDim, wireOffset, rectOffset);
        }
